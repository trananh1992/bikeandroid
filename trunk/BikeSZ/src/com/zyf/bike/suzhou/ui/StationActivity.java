...
import com.zyf.bike.suzhou.map.MyOverlay;
import com.zyf.bike.suzhou.map.StationOverlay;
...
//				return ServiceManager.getLives();
		...
		protected void onPostExecute(List<Station> result) {
			...
			if(result != null) {
				try {
					mMapView.getOverlays().add(new StationOverlay(getResources().getDrawable(R.drawable.marker_rounded_yellow_green), result));
					mMapView.refreshDrawableState();
				} catch (Exception e) {
					e.printStackTrace();
				}
			} else {
				Toast.makeText(StationActivity.this, "数据下载出错!", Toast.LENGTH_SHORT).show();
			}
		}
	}
}
